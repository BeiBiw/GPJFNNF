...
def caculate_sim(u, r, node, num2id, item_dict, word_vec, word_tokenizer, emb_dim=200):
    ...
    return sim
def get_slice2(zong_1,input,mask, tgt_batch):
    inputs, mask = input, mask
    items, n_node, A, alias_inputs = [], [], [], []
    for u_input in inputs:
        n_node.append(len(np.unique(u_input)))
    max_n_node = np.max(n_node)
    for z, u_input,tezheng in zip(range(len(inputs)), inputs,zong_1):
        ...
        for i in np.arange(len(u_input) - 1):
            ...
            u_A_in[u][v] = (tezheng[u_input[i]] * tezheng[u_input[i+1]]).sum(axis=1)
        ...
        #     u_A_in[i][i] = 1
        #     u_A_out[i][i] = 1
        u_A = np.concatenate([u_A_in, u_A_out]).transpose()  ###[6,3]->[3,6]
        A.append(u_A)
        alias_inputs.append([np.where(node == i)[0][0] for i in u_input])
    return alias_inputs, A, items, mask


def get_slice(input,mask, tgt_batch):
    inputs, mask = input, mask
    items, n_node, A, alias_inputs = [], [], [], []
    for u_input in inputs:
        n_node.append(len(np.unique(u_input)))
