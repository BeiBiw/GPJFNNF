...
class GPJFNNF(nn.Module):
    ...
    def forward(self, s1_input_ids, s2_input_ids,edge,tgt_batch,zong_ids):
        ...
        getR = lambda i: hidden_R[i][alias_inputs[i]]  ## 函数：在batch个hidden中，按照对应的alias_inputs的索引找出对应的隐藏特征
        seq_hidden2 = torch.stack([getR(i) for i in torch.arange(len(alias_inputs)).long()]) ##重新将他们拼接起来 [batch,node_num,hidden]
        H_global_R = self.gnn_R.compute_globalhidden(seq_hidden2, mask,num)  ##根据掩码将批次中每个节点的相关节点信息进行聚合来作为表征该节点的全局信息 [batch,hidden]
        s1_mask = torch.ne(s1_input_ids, 0)  ##判断s1_input_ids中哪个位置不=0（返回False）
